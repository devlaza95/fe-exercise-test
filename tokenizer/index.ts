...
type Success = { success: true; value: Token[]; rest: string };
...
const parseOperator: Parser = (input) => {
  const operators = ["+", "-"];
 ...
 */
const parseOpenParenthesis: Parser = (input) => {
  const firstChar = input[0];

  if (firstChar === "(") {
    const openParenthesisToken: Token = {
...
const parseCloseParenthesis: Parser = (input) => {
  const firstChar = input[0];

  if (firstChar === ")") {
    const closeParenthesisToken: Token = {
...
const parseOpenParenthesisResult4: Parser = parseCharacter(
  "(",
  "OPEN_PARENTHESIS"
);

const parseCloseParenthesisResult4: Parser = parseCharacter(
  ")",
  "CLOSE_PARENTHESIS"
);

console.log(
  "Parse open parenthesis result 4: ",
  parseOpenParenthesisResult4("(")
);
console.log(
  "Parse close parenthesis result 4: ",
  parseCloseParenthesisResult4(")")
);
//#endregion
...
const parseOperator2: Parser = choice(
...
function doUntil(parser: Parser): Parser {
  return (input: string) => {
    const recursiveParser: Parser = choiceN([
...
//#region Exercise 10
const tokenParsers: Parser[] = [
  parseNumber,
  parseOperator,
  parseOpenParenthesis,
  parseCloseParenthesis,
];
const tokenizer: Parser = choiceN(tokenParsers);
